...
boolean autoAddBlanks = false;
    ...
    private void initView(AttributeSet attrs) {
        if (attrs != null) {
            ...
            mTextSize = (int) ViewUtil.px2sp(typedArray.getDimension(R.styleable.BigBangLayout_textSize, ViewUtil.sp2px(settings.getPopupFontSize())));
            symbolSelectable = settings.getPopupSwitchSymbolSelection();
            ...
            mSymbolTextBgRes = typedArray.getResourceId(R.styleable.BigBangLayout_textBackground, DEFAULT_TEXT_BG_RES);
            mBlockSymbolBgRes = typedArray.getResourceId(R.styleable.BigBangLayout_symbolTextBackground, DEFAULT_SYMBOL_TEXT_BG_RES);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mNeedReDetectInMeasure) {
            ...
            for (int i = 0; i < childCount; i++) {
                ...
                if (currentLine.getItems() == null && (isEnter && showSection)) {
                ...
                } else if(item.isSymbol() && !StringUtil.isSpace(item.getText().toString())) {
                    ...
                    child.setBackgroundResource(mBlockSymbolBgRes);
                    child.setPadding(mSymbolTextPadding, mTextPaddingPort, mSymbolTextPadding, mTextPaddingPort);
                }else {
                    int padding = child.getPaddingLeft();
                    child.setBackgroundResource(mTextBgRes);
